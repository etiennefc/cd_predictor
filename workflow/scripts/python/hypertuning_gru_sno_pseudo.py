...
X = X[(X['predicted_label'] == 'CD_snoRNA') & (X['dataset'] == 'Tuning')]
y = X.copy()
...
input_size = len([col for col in X.columns 
            if col not in col_removal])  # number of SHAP input features (fixed_length + CLS + SEP)
...
num_epochs = 25 
batch_size = 32  # nb of example per batch 
...
def objective(trial):
    ...
    # Instantiate model and loss function (compute this loss function equally with regards to the 2 classes)
    ...
    for fold_index, (train_index, test_index) in enumerate(skf.split(x_tensor.numpy(), y_tensor.numpy())):
        ...
        with torch.no_grad():  # nor gradient computation
            ...
            pred_labels = torch.argmax(probabilities, dim=1)  # get the index (0, 1) of the highest probability
            ...
            # The 'macro' makes it that each class (0, 1) are taken into account equally (which is what we want)
