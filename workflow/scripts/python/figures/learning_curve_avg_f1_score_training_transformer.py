...
dfs = []
val = [random.uniform(0, 0.4) for _ in range(10)]

print(snakemake.input.f1_score_tsv)
for i, p in enumerate(snakemake.input.f1_score_tsv):
    df = pd.read_csv(p, sep='\t')
    new_row = pd.DataFrame({'f1_score_per_epoch': [val[i]]})  # before first epoch (have to change it for real predicted value before training)
