...
expressed_paths = glob.glob(
                f'{snakemake.input.tgirt_dir}/*expressed_snoRNAs*')
pseudo_paths = glob.glob(
                f'{snakemake.input.tgirt_dir}/*pseudogene_snoRNAs*')
...
# Create bar chart
ft.stacked_bar2(percent_counts, species_short,
                exp_status, '', 'Species name', 
                'Proportion of C/D\nbox snoRNAs (%)', colors, 0, 102, 
                [''] * len(sno_nb), snakemake.output.bar) 
