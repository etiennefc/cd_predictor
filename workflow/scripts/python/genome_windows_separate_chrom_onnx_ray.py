...
batch_size = 16
num_labels = 2
output = snakemake.output.windows
...
def predict(initial_batch_index, big_b, chr_dictio, chr_name, chr_seq, window_size, mini_batch_size, strand_name, mem_batch=10):
    # Load model in onnx for first batch only to speed up prediction after
    if initial_batch_index == 0:
        print('\n\nMODELTEMPLOAD\n\n')
        start_time = time.time()
        first_chrom = list(chr_dict.keys())[0]
        for big_batch in scan_fasta(chr_dict, chr_dict[first_chrom], window_size, batch_size):
                for i, ev_batch in enumerate(big_batch[0]):
                    ...
                    with torch.no_grad():  # nor gradient computation
                        torch.onnx.export(model, (ev_input_ids, ev_attention_mask), f"model_temp.onnx", input_names=["input_ids", "attention_mask"], output_names=["output"])
                        ort_session = onnxruntime.InferenceSession(f"model_temp.onnx")
                        if i == 0:
                            break
                break
        end_time = time.time()
        ...
        with torch.no_grad():
            ort_session = onnxruntime.InferenceSession(f"model_temp.onnx")
        end_time = time.time()
        print(f'Load model SECOND in onnx : {end_time -st_time}s')
    total_window = ceil(((len(chr_dictio[chr_name]) - int(window_size))/int(step_size)) + 1)
    print(total_window)
    all_positives = []
    n_batch = 0
    #for big_batch in scan_fasta(chr_dictio, chr_seq, window_size, mini_batch_size, mem_batch_size=mem_batch):
    for i, ev_batch in enumerate(big_b[0]):
        n_batch += 1
        ...
        with torch.no_grad():  # nor gradient computation
            ...
            if len(ev_input_ids_np) != mini_batch_size:  # to account for the last batch at the end of chromosome
                ...
                if 1 in pred_labels:  # i.e. positive predictions
                    ...
                    starts = big_b[1][i*mini_batch_size:i*mini_batch_size+mini_batch_size].to(device)
                    pos_starts = starts[positives_index].to(device)
                    if pos_starts.dim() == 0:
                        pos_starts = pos_starts.unsqueeze(0)
                    ends = big_b[2][i*mini_batch_size:i*mini_batch_size+mini_batch_size].to(device)
...
pred_id = ray.put(predict)

# Decorate the predict function with the decorator ray.remote (basically a function) 
@ray.remote
def test_pred(predict, initial_batch_index, big_b, chr_dictio, chr_name, chr_seq, window_size, mini_batch_size, strand_name):
    return predict(initial_batch_index, big_b, chr_dictio, chr_name, chr_seq, window_size, mini_batch_size, strand_name)
...
if strand != 'both':
    ...
    initial_batch_index = len(glob.glob('model_temp.onnx'))
    chromo_name = 'Ca22chr4A_C_albicans_SC5314'
    results_df = ray.get([test_pred.remote(pred_id, initial_batch_index, big_b, chr_dict, chromo_name, chr_dict[chromo_name], 
                        window_size, batch_size, s) for big_b in scan_fasta(chr_dict, chr_dict[chromo_name], 
                        window_size, batch_size)])
    results_df = results_df.sort_values(by=['chr', 'start', 'end'])
    if strand == 'negative':
        ...
        print(results_df)
    end_time = time.time()
    sp.call('rm model_temp*.onnx', shell=True)
