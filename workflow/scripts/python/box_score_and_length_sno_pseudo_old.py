...
df['len'] = df.D_end + 5 - df.C_start + 5
ddf['len'] = ddf.D_end + 5 - ddf.C_start + 5
expressed_cd_df['len'] = expressed_cd_df.end - expressed_cd_df.start + 1
print(df[['gene_id', 'C_start', 'D_end', 'len']].sort_values(by='len'))
print(ddf[['gene_id', 'C_start', 'D_end', 'len']].sort_values(by='len'))
pseudo_df = pd.read_csv('/home/etienne/Desktop/Etienne/pseudo.tsv', sep='\t')
print(pseudo_df.dtypes)
print(pseudo_df[['start', 'end']])
pseudo_df[['start', 'end']] = pseudo_df[['start', 'end']].astype(float)
pseudo_df['len'] = pseudo_df.end - pseudo_df.start + 1
import matplotlib.pyplot as plt
import seaborn as sns
sns.kdeplot(df.len, fill=True, color='green')
sns.kdeplot(expressed_cd_df.len, fill=True, color='lightgreen')
sns.kdeplot(ddf.len, fill=True, color='red')
sns.kdeplot(pseudo_df.len, fill=True, color='pink')
plt.savefig('temp_len.svg', bbox_inches='tight', dpi=500)
