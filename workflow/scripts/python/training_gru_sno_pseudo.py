...
X = X[(X['predicted_label'] == 'CD_snoRNA') & (X['dataset'] == 'Training')]
y = X.copy()
...
num_epochs = 125 
batch_size = 96  # nb of example per batch (this is an intermediate batch size)
...
for fold_index, (train_index, test_index) in enumerate(skf.split(x_tensor.numpy(), y_tensor.numpy())):
    ...
    loss_fn = nn.CrossEntropyLoss(weight=torch.tensor([1, 1/2]))
        ...
        with torch.no_grad():  # nor gradient computation
            ...
            if epoch == (num_epochs - 1):
                ...
                metrics_df = pd.DataFrame([[fold_i, accuracy, fscore, precision_sno, 
                                        recall_sno, precision_pseudosno, recall_pseudosno]], 
                                        columns=['fold', 'accuracy_2_classes', 'f1_score_2_classes',
...
avg = ['average_fold']
avg = avg + list(final_metrics_df[['accuracy_2_classes', 'f1_score_2_classes',
