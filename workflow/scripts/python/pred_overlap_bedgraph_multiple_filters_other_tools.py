...
tool = str(snakemake.wildcards.cd_predictors)
...
bedgraphs_sorted_pos = [
    f'final_{species}_'+p for p in bedgraph_names if p.endswith(
        'fwd.bedgraph')]
...
if species == 'homo_sapiens':
    ...
    for i, b in enumerate(bedgraphs_neg):  # remove unwanted last line
        ...
        shell=True)
pred_path = snakemake.input.preds
preds = pd.read_csv(pred_path, sep='\t', names=['chr', 'start', 'end', 'gene_id', 'score', 'strand'], 
            dtype={'chr': 'str'})
total_preds = len(preds)
if species != 'tetrahymena_thermophila':
    preds['chr'] = preds['chr'].apply(lambda x: f'chr{x}' if not str(x).startswith('chr') else x)
    preds['chr'] = preds['chr'].astype(str)
else:
    preds['chr'] = preds['chr'].astype(str) 
preds['gene_length'] = preds['end'] - preds['start']

# Gene length should not exceed 164 nt to compare with snoBIRD predictions
preds = preds[preds['gene_length'] <= fixed_length - 30]
...
annotated_and_pred = bed_cd.intersect(preds_bed, s=True, wb=True, 
                ...
                'strand_pred'])
# Get rid of overlapping predictions (those overlapping the same locus)
annotated_and_pred = annotated_and_pred.drop_duplicates('known_cd_id')
num_annotated_and_pred = len(annotated_and_pred)

print(num_annotated_and_pred)
...
pivot_df = grouped_df.pivot_table(index=[
                        ...
                        fill_value=0).reset_index()
print(pivot_df)                    
#pivot_df.to_csv('human_pivot1.tsv', index=False, sep='\t')
...
if species in high_rep_species:  
    ...
    for n in _names:
        ...
        pivot_df[n+'_rev__AVG'] = pivot_df.filter(
                            regex=n+'_[0-9]*(.bam)?_rev.bedgraph$').mean(axis=1)
    print(pivot_df)
    #pivot_df.to_csv('human_pivot2.tsv', index=False, sep='\t')
...
# Get rid of overlapping predictions (those overlapping the same locus)
all_pred_pos_overlap = list(pos_overlap_extended.gene_id)
pos_overlap_extended = pos_overlap_extended.drop_duplicates('known_cd_id')
pos_overlap_extended_bed = BedTool.from_dataframe(pos_overlap_extended)

pos_overlap_extended_bed.igv(name=True, path=screenshot_dir+"known_cd", img="png")
sp.call(f'mv {pos_overlap_extended_bed.igv_script} {snakemake.output.batch_script_known_cd}', shell=True)
...
potential_new_cd = pivot_df_cov_flank[~pivot_df_cov_flank['gene_id'].isin(all_pred_pos_overlap)].reset_index(drop=True)
...
final_candidates = exp_preds[exp_preds['gene_id'].isin(cand_list)].sort_values(['chr', 'start'])

# Get rid of overlapping predictions over the same locus by merging the predictions
# save values before merging overlapping entries
final_candidates.drop(columns=['dot']).to_csv(snakemake.output.new_cd_df, sep='\t', index=False)
final_candidates_bed = BedTool.from_dataframe(final_candidates[['chr', 'start', 'end', 'gene_id', 'avg_coverage_samples', 'strand']])
if len(final_candidates) > 0:
    merged_final = final_candidates_bed.merge(s=True, c=[4, 5, 6], o=['distinct', 'mean', 'distinct'])

    final_candidates = merged_final.to_dataframe(names=['chr', 'start', 'end', 'gene_id', 'avg_coverage_samples', 'strand'])
final_candidates = final_candidates.sort_values('avg_coverage_samples').reset_index(drop=True)


print(final_candidates)
bed_final = BedTool.from_dataframe(final_candidates[['chr', 'start', 'end', 'gene_id', 'avg_coverage_samples', 'strand']])
...
print(species)
print(f'Total number of {tool} predictions: {total_preds}')
print(f'Total number of expressed {tool} predictions (avg coverage >=5 and flanking reads filters): '+
...
# Info df
cols = ['species', 'total_annot_cd', 'total_annot_exp', f'total_{tool}_preds', 
        'total_annot_preds', f'exp_{tool}_preds_5_reads_and_filters', 
        'exp_annot_preds', 'potential_novel_expressed']
final_vals = [species, len(cd_df_pos) + len(cd_df_neg), num_annot_expressed, 
                total_preds, num_annotated_and_pred, 
...
sp.call(f'rm final_{species}* exp_preds_* flanking_pos* flanking_neg*', shell=True)
