rule hypertuning_transformer_2_classes:
    ...
    input:
        X_tuning = rules.get_three_sets_initial_fixed_length_data_aug.output.tuning,  # 1_ratio
        y_tuning = rules.get_three_sets_initial_fixed_length_data_aug.output.tuning_target  # 1_ratio
    output:
...
rule training_transformer_2_classes_LR_schedule:
    """ Train SnoBIRD's first model with the best hyperparams. Must be connected 
        to internet to load the pretrained model for the first time. The learning 
        rate is first linearly warmed-up from 0 to 4e-5 in the first epoch, and 
        then linearly decayed to 0 at the last epoch of training"""
    input:
        X_train = rules.get_three_sets_initial_fixed_length_data_aug.output.training,  # 1_ratio
        y_train = rules.get_three_sets_initial_fixed_length_data_aug.output.training_target,  # 1_ratio
...
rule test_before_training_transformer_2_classes:
    ...
    input:
        X_train = rules.get_three_sets_initial_fixed_length_data_aug.output.training,        
        y_train = rules.get_three_sets_initial_fixed_length_data_aug.output.training_target,
        best_hyperparams = rules.hypertuning_transformer_2_classes.output.best_hyperparams
    output:
...
rule test_transformer_2_classes_LR_schedule:
    ...
    input:
        X_test = rules.get_three_sets_initial_fixed_length_data_aug.output.test,
        y_test = rules.get_three_sets_initial_fixed_length_data_aug.output.test_target,
        best_hyperparams = rules.hypertuning_transformer_2_classes.output.best_hyperparams,
