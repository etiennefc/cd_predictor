...
rule random_shuffle_sno_fixed_length_520nt:
    """ Shuffle expressed C/D sequences and use that as 
        negative examples from all species"""
    input:
        sno_sequences = rules.get_sno_sequences_fixed_length_520nt.output.df
    output:
    ...
    script:
        "../scripts/python/random_shuffle_sno_fixed_length_520nt.py"

rule filter_rnacentral_tRNA_snRNA_pre_miRNA_fixed_length_520nt:
    """ Filter rnacentral ncRNA beds (keep non-overlapping entries 
        ...
        to obtain 520 nt (fixed length as with the C/D snoRNAs)."""
    input:
    ...
    conda:
        "../envs/python_new.yaml"
    script:
        "../scripts/python/filter_rnacentral_tRNA_snRNA_pre_miRNA_fixed_length_520nt.py"

rule format_blat_haca_output_fixed_length_520nt:
    """ Format BLAT output to keep only the match with the highest 
        ...
        and potential mismatches mainly) and extend to 520 nt of length."""
    input:
        blat = rules.blat_haca_genome.output.sno_location,
        genome = get_species_genome,
        chr_size = get_chr_size
    output:
        df = 'data/references/HACA/{species}_HACA_location_formated_fixed_length_520nt.tsv'
    params:
    ...
    conda:
        "../envs/python_new.yaml"
    script:
        "../scripts/python/format_blat_haca_output_fixed_length_520nt.py"

rule select_random_intergenic_intronic_exonic_regions_fixed_length_520nt:
    ...
    input:
        ...
        genome = get_species_genome
    output:
        random_intronic_regions = 'data/references/negatives/random_regions/selected_intronic_regions_{species}_fixed_length_520nt.bed',
        random_intergenic_regions = 'data/references/negatives/random_regions/selected_intergenic_regions_{species}_fixed_length_520nt.bed',
        random_exonic_regions = 'data/references/negatives/random_regions/selected_exonic_regions_{species}_fixed_length_520nt.bed'
    params:
        random_state = 42
    conda:
        "../envs/python_new.yaml"
    script:
        "../scripts/python/select_random_intergenic_intronic_exonic_regions_fixed_length_520nt.py"

rule get_all_initial_negatives_fixed_length_520nt:
    ...
    input:
        ncRNA = rules.filter_rnacentral_tRNA_snRNA_pre_miRNA_fixed_length_520nt.output.df,
        haca = expand(rules.format_blat_haca_output_fixed_length_520nt.output.df, 
                    species=[sp for sp in config['species']+config['species_tgirt'] 
                        if sp not in ['ostreococcus_tauri', 'schizosaccharomyces_pombe', 
                        'tetrahymena_thermophila']]),
        shuffle_sno = rules.random_shuffle_sno_fixed_length_520nt.output.shuffled_sno_df,
        random_intronic_regions = expand(rules.select_random_intergenic_intronic_exonic_regions_fixed_length_520nt.output.random_intronic_regions,
                    species=[sp for sp in config['species']+config['species_tgirt'] 
                        if sp not in ['ostreococcus_tauri', 'schizosaccharomyces_pombe']]),
        random_intergenic_regions = expand(rules.select_random_intergenic_intronic_exonic_regions_fixed_length_520nt.output.random_intergenic_regions,
                    species=[sp for sp in config['species']+config['species_tgirt'] 
                        if sp not in ['ostreococcus_tauri', 'schizosaccharomyces_pombe']]),
        random_exonic_regions = expand(rules.select_random_intergenic_intronic_exonic_regions_fixed_length_520nt.output.random_exonic_regions,
                    species=[sp for sp in config['species']+config['species_tgirt'] 
                        if sp not in ['ostreococcus_tauri', 'schizosaccharomyces_pombe']]),
    ...
    output:
        tuning = 'data/references/negatives/initial/negatives_tuning_set_fixed_length_520nt.tsv',
        training = 'data/references/negatives/initial/negatives_training_set_fixed_length_520nt.tsv',
    ...
    conda:
        "../envs/python_new.yaml"
    script:
        "../scripts/python/get_all_initial_negatives_fixed_length_520nt.py"
